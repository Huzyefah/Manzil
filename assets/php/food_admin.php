...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_product'])) {
    ...
    $sql = "INSERT INTO food (name, price, type, rating, link, imageURL) VALUES ('$name', '$price', '$type', '$rating', '$link', '$imageURL' )";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_product'])) {
    ...
    if (isset($_POST['selected_product'])) {
        ...
        $sql = "DELETE FROM food WHERE id = '$selected_product'";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<option value='{$row['id']}'>{$row['name']} (Rating: {$row['rating']}, Price: {$row['price']})</option>";
    }
    echo "</select><br>";
